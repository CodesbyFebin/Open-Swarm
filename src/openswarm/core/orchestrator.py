...
class SwarmOrchestrator:
    ...
    async def scout_node(self, state: SwarmState) -> Dict[str, Any]:
        ...
        # Route to fast model
        model = await route_task(f"Explore codebase for: {state['goal']}")
    ...
    async def planner_node(self, state: SwarmState) -> Dict[str, Any]:
        ...
        findings = scout_entries[-1].value if scout_entries else {}
        
        # Route to reasoning model
        model = await route_task(f"Plan: {state['goal']}")
        
        # Generate plan
        plan = f"""1. Analyze current auth.py implementation
...
6. Verify compatibility"""
        ...
        return {
            ...
            "workflow_stage": "planner_complete"
        }
    ...
    async def parallel_worker_node(self, state: SwarmState) -> Dict[str, Any]:
        ...
        coder_result, critic_result = await asyncio.gather(
            coder_task,
            critic_task
        )
    ...
    async def _coder_subtask(self, state: SwarmState, plan: str) -> Dict[str, Any]:
        ...
        model = await route_task(f"Code: {plan}")
...
"""
    ...
    async def _critic_subtask(self, state: SwarmState, plan: str) -> Dict[str, Any]:
        ...
        model = await route_task(f"Critique plan: {plan}")
...
"""
    ...
    async def synthesizer_node(self, state: SwarmState) -> Dict[str, Any]:
        ...
        model = await route_task(f"Synthesize: {state['plan']}")
...
Ready for implementation."""
    ...
    async def run_swarm(self, goal: str, config: Optional[Dict] = None) -> Dict[str, Any]:
        """
        ...
        try:
            # Stream events
            async for event in self.workflow.astream(initial_state, {"configurable": {"thread_id": thread_id}}):
