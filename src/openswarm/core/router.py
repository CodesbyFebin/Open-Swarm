...
class SwarmRouter:
    ...
    async def assess_task(self, task_description: str) -> TaskAssessment:
        ...
        if any(word in task_lower for word in ['explore', 'find', 'search', 'list', 'grep', 'read']):
            domain = "exploration"
            complexity = "low"
        elif any(word in task_lower for word in ['refactor', 'rewrite', 'implement', 'code', 'function']):
            domain = "coding"
            complexity = "medium"
        ...
        requires_parallel = any(word in task_lower for word in ['multiple', 'parallel', 'simultaneous', 'several'])
    ...
    def _filter_available_models(self, purpose: str) -> List[ModelProfile]:
        ...
        if self.config and self.config.enable_rate_limit_awareness:
            ...
            candidates = [
                ...
                if m.last_429 is None or 
                   now - m.last_429 > timedelta(seconds=m.rate_limit_cooldown)
            ]
    ...
    async def route_task(self, task_description: str) -> str:
        ...
        print(f"[Router] Task assessed as {assessment.complexity}/{assessment.domain}. "
